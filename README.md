...
Stage 1会给整份文件标注一个篇章级领域（domain），用于翻译时的语境提示；Stage 2抽取术语时会
再为每条术语单独判断领域——大多数术语跟篇章领域一致，但文中偶尔出现的跨领域术语（例如法律
文档里夹的技术术语）会被标注成它自己的领域，不会被强行归到篇章领域。领域均从同一份封闭词表
中选取，模型不会自造新词；选不出合适项时标记为「待归类」并记录建议。TUI首页「领域词表管理」
可逐条修改名称后收录或驳回，也可打开内置、个人及待归类JSONL文件直接编辑；重复名称不会收录，
原待归类记录会保留。命令行也可查看、转正。本地术语库按（原文术语＋领域）区分同词不同义，同一领域内如果确实
出现多个不同译法也都会保留，不互相覆盖。Stage 4审阅表设有「领域」列，可查看并修改每条术语的
领域判断；修改时下拉限制在封闭词表内（含「待归类」），也可填写词表外的领域名，此时会自动记
入待归类队列，走与Stage 1相同的转正流程。
