...
Stage 2抽取术语后先识别无需翻译的项目；其余候选在Stage 3依次查证：

| 顺序 | 层级 | 规则 |
|---|---|---|
| 0 | 不译 | 固定写法原样保留，不进入后续查证 |
...
以及Codex CLI和Claude Code CLI。CLI采用批量查证，单次调用最长五分钟；未知调用格式按联网失败处理。

TUI显示各层数量及证据等级；术语审阅表保存所选证据及其来源URL，完整检索记录写入项目证据文件。

本地层先检查项目人工覆盖，再查询本地术语库；各类命中统一标记为“本地”。

| 本地来源 | 优先级 | 规则 |
|---|---|---|
| 项目人工覆盖 | 高 | 仅在当前项目生效，不回写术语库 |
| TRANSilk 内部库 | 同级 | 资产生成并通过校验后，自动同步其中可复用的首选术语 |
| 外部 TBX | 同级 | 由用户手动挂载，不与内部库合并 |

内部库及挂载登记均位于安装目录下，不依赖固定盘符。

每个项目在 `99_项目配置与术语源数据/项目专用译法.xlsx` 提供可选的人工覆盖入口，固定填写
“原文术语”和“指定译文”两列。TUI 项目页可直接打开该文件；填写的术语优先采用，原文中存在但
未被 Stage 2 抽取的术语会补入候选。重复原词或缺失字段会按行报错，原文未出现的条目仅作提示。
项目专用译法保留在人类工作簿中，但不进入 TBX、JSONL 或 TRANSilk 内部库。
...
用于修改术语领域。本地术语库按“语言方向＋原文术语＋领域”区分词义，并允许保留必要的多个译法。

**检查更新**

TUI 每 24 小时在后台检查一次更新；自动检查只提示新版本，不修改工作区，离线或超时也不影响使用。
在「其他设置」中选择「检查并安装更新」（或运行 `transilk check-update`）后，TRANSilk 才会拉取
上游提交。更新前会检查源码状态；存在未提交改动或历史分叉时停止。依赖锁文件发生变化时会重新
安装依赖，随后验证程序能否启动；失败时恢复更新前的代码和依赖。项目、术语库、API 密钥及个人
领域词表不参与代码更新和回滚。访问 GitHub 受限时，需自行配置代理，例如设置 `HTTP_PROXY`
或 `HTTPS_PROXY` 环境变量。
...
`术语源数据.jsonl`保留当前项目的审计记录和翻译约束；归档生成的术语工作簿、TBX、JSONL
及本地术语库仅收录经人工确认且可复用的首选术语。
...
# 6. 可选：手动管理外部术语库挂载
node src/cli.mjs mount-termbase ./my-termbase.tbx
node src/cli.mjs list-termbases
node src/cli.mjs unmount-termbase <挂载ID或路径>
...
- 外部术语库仅支持手动挂载 TBX，按语言方向、原词和领域精确匹配；不支持 TMX 或模糊匹配。
